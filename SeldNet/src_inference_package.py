...
def create_src_inference_zip(
    ...
    best_weights_src_path: str,
    training_log_src_path: str,
    params: Mapping[str, Any],
    task_id: str,
    *,
    unique_name: str,
    ...
    """
    Создаёт zip: model.conf, scaler.joblib, README.txt
    + артефакты обучения (best/final checkpoints и лог).

    weights_src_path — обычно *_model_final.h5 из train_seldnet.
    best_weights_src_path — обычно *_model.h5 из train_seldnet.
    training_log_src_path — обычно logs/<job-id>.txt.
    ...
    if not os.path.isfile(best_weights_src_path):
        raise FileNotFoundError(f"Нет файла лучших весов: {best_weights_src_path}")
    if not os.path.isfile(training_log_src_path):
        raise FileNotFoundError(f"Нет лога обучения: {training_log_src_path}")
    ...
    with tempfile.TemporaryDirectory(prefix="src_pack_") as tmp:
        final_ckpt_name = os.path.basename(weights_src_path)
        ...
        write_model_conf(
            ...
            model_filename=final_ckpt_name,
        ...
        with open(readme, "w", encoding="utf-8") as f:
            f.write(
                ...
                "   models/<имя_модели>/\n"
                f"2. В каталоге должны лежать: model.conf, {final_ckpt_name}, scaler.joblib\n"
        ...
        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
            for name in ("model.conf", "scaler.joblib", "README.txt"):
                fp = os.path.join(tmp, name)
                zf.write(fp, arcname=name)
            # Добавляем исходные файлы чекпоинтов и лог без переименования.
            zf.write(weights_src_path, arcname=os.path.basename(weights_src_path))
            zf.write(best_weights_src_path, arcname=os.path.basename(best_weights_src_path))
            zf.write(training_log_src_path, arcname=os.path.basename(training_log_src_path))
