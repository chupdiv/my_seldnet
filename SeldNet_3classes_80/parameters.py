...
#
# This copy (SeldNet_3classes_80) targets clips of 80 ms total duration with label frames of 40 ms (2 label frames).
# STFT hop is hop_len_s=0.04 (40 ms) => feature_label_resolution=1, feature_sequence_length=2 per chunk.
#
...
def get_params(argv='1'):
    ...
    params = dict(
        ...
        # FEATURE PARAMS (80 ms аудио на чанк = 2 метки по 40 ms; STFT hop 40 ms → 2 спектральных кадра)
        fs=FS,
        hop_len_s=0.02,
        ...
        nb_epochs=800,  # Train for maximum epochs
    ...
    if argv == '1':
        print("USING DEFAULT PARAMETERS (80 ms, task 6-style timing)\n")
        ...
        print("MIC + GCC + multi ACCDOA (80 ms clips, 2x40 ms label frames)\n")
        params['quick_test'] = False
        params['dataset'] = 'mic'
        params['use_salsalite'] = False
        params['multi_accdoa'] = True
        ...
        # At fs=250 kHz, max |TDOA| for ~0.14 m baseline is ~103 samples; must be <= max_tau.
        params['max_tau'] = 128
        params['tracks'] = 3
        ...
        print("QUICK TEST MODE (80 ms timing, same as default/task 6)\n")
