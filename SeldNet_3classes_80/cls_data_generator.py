...
class DataGenerator(object):
    def __init__(self, params, split=1, shuffle=True, per_file=False, is_eval=False):
        ...
        # ПРИМЕЧАНИЕ: Если датасет содержит файлы по 15 сек, они НЕ нарезаются на части заранее.
        # Вместо этого:
        # 1. Файл загружается целиком (или извлекаются чанки через _audio_chunks_from_file, если raw_chunks=True)
        # 2. Признаки разбиваются на последовательности длиной feature_sequence_length кадров
        #    (по умолчанию 2 кадра = 80 мс при hop_len_s=0.02 и label_sequence_length=5)
        # 3. Метки разбиваются на последовательности длиной label_sequence_length кадров
        #    (по умолчанию 5 кадров = 200 мс при label_hop_len_s=0.04)
        # Таким образом, 15-секундный файл порождает множество перекрывающихся или неперекрывающихся
        # обучающих примеров (чанков) фиксированной длины для подачи в сеть.
    ...
    def _split_in_seqs(self, data, _seq_len): # data - 250*8, 7, 64 - 250
        # ПРИМЕЧАНИЕ ДЛЯ argv=6: Эта функция нарезает признаки/метки из длинного файла (например, 15 сек)
        # на последовательности фиксированной длины (_seq_len кадров), которые подаются в сеть.
        # Для argv=6:
        #   - Признаки: _seq_len = feature_sequence_length = 2 кадра (80 мс)
        #     15-секундный файл → ~750 кадров признаков → 375 последовательностей по 2 кадра
        #   - Метки: _seq_len = label_sequence_length = 5 кадров (200 мс)
        #     15-секундный файл → ~375 кадров меток → 75 последовательностей по 5 кадров
        # Таким образом, один длинный файл разбивается на множество коротких обучающих примеров.
        if len(data.shape) == 1:
