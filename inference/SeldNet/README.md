...
## This copy: `SeldNet_3classes_80` (80 ms clips)

This directory is a self-contained fork for training on **80 ms** WAV segments with **40 ms label frames** (two label steps per clip). All original task ids run in 80 ms mode (`label_hop_len_s=0.04`, `label_sequence_length=2`) except task `9`, which keeps `label_sequence_length=1` for TDOA pretraining on short clips. `hop_len_s=0.04`, so standard tasks use `feature_sequence_length=2` STFT hops per sample. Set `dataset_dir` / `feat_label_dir` in `parameters.py` to your data; `feat_label_dir` defaults to a `*_seld_feat_label_80ms/` path.

**GCC + MS** (80 ms): `python batch_feature_extraction.py 6` then `python train_seldnet.py 6 <job-id> <seed>`.

Original task names are preserved: `1, 2, 3, 4, 5, 6, 7, 9, 10, 32, 33, 34, 333, 999`.

**CST-Former ULE on 80 ms:** use original task ids **`33`** (GCC+MS), **`34`** (SALSA-Lite), **`333`** (NGCC+MS). ULE unfold/fold sizes are derived from `feature_sequence_length` and pooling in `cst_former/CST_details/cst_geometry.py` (replaces fixed 50×64 / patch 10×4 constants).
