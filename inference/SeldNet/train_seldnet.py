...
class TdoaLoss(nn.Module):
    def __init__(self, fs=24000, c=343, mic_locs = MIC_LOCS_DEFAULT, ntdoas=6, max_tau=6, tracks=5,):
        ...
        # Cartesian mic positions (m), same geometry as MIC_GEOMETRIES[4] in
        # generate_reverb_all_inclusive_gpu_voice.py: square in the YZ plane, edge length 0.1 m
        # (apex of the "pyramid" lies on +X; base is the square in YZ — matches gpuRIR simulation).
        self.mic_locs = torch.tensor(np.array(mic_locs),
            dtype=torch.float32,
        )
                        ...
                        if active:
                            ...
                            for m1 in range(self.nmics):
                                for m2 in range(m1+1, self.nmics):
                                    ...
                                    tdoa = max(-self.max_tau, min(self.max_tau, tdoa))
...
def main(argv):
    ...
    if params['mode'] == 'dev':
        ...
        log_file_path = os.path.join('logs/', job_id + '.txt')
        ...
        for split_cnt, split in enumerate(test_splits):
            ...
            if params['predict_tdoa']:
                criterion_tdoa = TdoaLoss(
                    fs=params['fs'], 
                    max_tau=params['max_tau'], 
                    tracks=params['tracks'],
                    mic_locs=params['mic_locs']
                )
            else:
                criterion_tdoa = None
            ...
            for epoch_cnt in range(nb_epoch):
                ...
                if patience_cnt > params['patience']:
                    log_string("saving final model (early stopping)")
                    torch.save(model.state_dict(), model_name_final)
                    break
            ...
            try:
                import src_inference_package
                _zip_path = os.path.join(params['model_dir'], '{}_src_inference.zip'.format(unique_name))
                src_inference_package.create_src_inference_zip(
                    _zip_path,
                    model_name_final,
                    model_name,
                    log_file_path,
            ...
            except Exception as _src_pack_err:
                log_string('src_inference_package: не удалось создать архив для uav_acoustic_classification (src): {}'.format(_src_pack_err))
